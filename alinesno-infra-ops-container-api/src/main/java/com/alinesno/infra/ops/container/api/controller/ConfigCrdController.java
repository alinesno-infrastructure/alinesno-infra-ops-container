...
/**
 * 处理与ConfigCrdEntity相关的请求的Controller。
 * 继承自BaseController类并实现IConfigCrdService接口。
 * 
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class ConfigCrdController extends BaseController<ConfigCrdEntity, IConfigCrdService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(ConfigCrdController.class);

    @Autowired
    private IConfigCrdService service;

    /**
     * 获取ConfigCrdEntity的DataTables数据。
     *
    ...
    @Override
    public IConfigCrdService getFeign() {
        return this.service;
    }
}
