...
/**
 * 处理与EnvNodesEntity相关的请求的Controller。
 * 继承自BaseController类并实现IEnvNodesService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class EnvNodesController extends BaseController<EnvNodesEntity, IEnvNodesService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(EnvNodesController.class);

    @Autowired
    private IEnvNodesService service;

    /**
     * 获取EnvNodesEntity的DataTables数据。
     *
    ...
    @Override
    public IEnvNodesService getFeign() {
        return this.service;
    }
}
