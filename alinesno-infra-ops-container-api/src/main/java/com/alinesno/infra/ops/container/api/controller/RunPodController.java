...
/**
 * 处理与RunPodEntity相关的请求的Controller。
 * 继承自BaseController类并实现IRunPodService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
public class RunPodController extends BaseController<RunPodEntity, IRunPodService> {
    ...
    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(RunPodController.class);

    @Autowired
    private IRunPodService service;

    /**
     * 获取RunPodEntity的DataTables数据。
     *
    ...
    @Override
    public IRunPodService getFeign() {
        return this.service;
    }
}
